...
void buff_detect:: machineLearningF(Mat image, RotatedRect father, vector<RotatedRect>&blade, Ptr<SVM> svm)
{
	Rect roi;
	Mat target;
	Mat target_t;
	...
	Mat p = target_t.reshape(1, 1);
	p.convertTo(p, CV_32FC1);
	response = (int)svm->predict(p);//开始对每张图片进行预测，同时进行分类响应，针对于某一张图片预测后得到的响应只有一个

	if (response == 1)
	{
		blade.push_back(father);
		//char imagename[100];
		//sprintf_s(imagename, "%s%d.jpg", "./blade/Pos/Ri", num);
		//resize(target, target, Size(48, 48));
		//imwrite(imagename, target);
		//num++;
	...
	//{
	//	char imagename[100];
	//	sprintf_s(imagename, "%s%d.jpg", "./blade/Neg/Ru", num);
	//	resize(target, target, Size(48, 48));
	//	imwrite(imagename, target);
	//	num++;
	//}	
